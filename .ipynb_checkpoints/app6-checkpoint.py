...
def extract_text_from_pdf(uploaded_file) -> str:
    """Text-only extraction. Returns empty string for scanned/encoded-font PDFs."""
...
def is_garbled_text(text: str) -> bool:
    """
    Returns True if extracted text is garbled due to custom/encoded fonts.
...
def pdf_to_image_bytes(pdf_bytes: bytes, zoom: float = 2.0) -> tuple[bytes, list[bytes]]:
    ...
    Returns (stitched_png_bytes, [page1_png_bytes, page2_png_bytes, ...])
    The stitched image is used for the vision model; individual pages for display.
    """
    import fitz
    ...
    # Stitch all pages vertically for vision model
...
def extract_date_from_pdf_top(pdf_bytes: bytes, zoom: float = 2.0) -> str | None:
    """
    Extract the purchase date from a PDF receipt.
    Strategy:
    - Scan all pages via PyMuPDF text layer for MM/DD/YYYY or M/D/YY patterns
    - Prefers dates near "APPROVED" or timestamp patterns (in-store receipts)
    - Falls back to first date found anywhere
    """
    try:
        import fitz
        DATE_RE = re.compile(r"\b(\d{1,2})/(\d{1,2})/(\d{2,4})\b")

        def parse_date_match(match) -> str | None:
        ...
        # Look for date on same line as APPROVED or timestamp (in-store receipt pattern)
        for line in all_text.splitlines():
            if re.search(r"APPROVED|AMOUNT|\d{4}\s+\d{3}\s+\d{3}", line, re.IGNORECASE):
                m = DATE_RE.search(line)
                if m:
                    result = parse_date_match(m)
                    if result:
                        return result

        # Fallback: first valid date anywhere in the text
    ...
    """
    Try to extract a date by scanning all pages text layers via PyMuPDF.
    Looks for patterns like 2/27/26 or 2/27/2026 on any page.
...
You must return:
- store (string)
- receipt_date (YYYY-MM-DD) — Find the purchase date carefully:
  * For Costco WEB/PDF receipts: date is in the TOP-LEFT corner of page 1 in M/D/YY format (e.g. "2/27/26" = 2026-02-27)
  * For Costco IN-STORE receipts: date appears near the bottom, next to "APPROVED" or as a timestamp like "02/23/2026 15:38" in MM/DD/YYYY format
  Use whichever date is present. Do NOT use dates from order detail sections. Do NOT invent or default a date.
- subtotal (number)
- tax (number)
- total (number)
...
- If store is Costco: an item is taxable ONLY if the receipt shows a "Y" marker next to that item line -> set taxable=true. Items marked "N" are not taxable.
- Costco discount rule: A line like "357897 /2189436 4.00-" means a $4.00 discount on item 2189436. Represent this as a DISCOUNT line immediately after that item.
- Discount lines are NOT items.
- Keep item "price" as the base price (before discount). Net price is computed in code.
...
if os.path.exists(DB_PATH) and inspect(engine).has_table("receipts"):
    ...
    if not items_df.empty:
        ...
        if selected_item_label:
            ...
            if st.sidebar.button("Update Item Category", width="stretch"):
...
if input_kind in ("camera", "image") and image_bytes:
    st.image(image_bytes, caption=upload_preview_name or "Receipt image", width=300)
        ...
        try:
            if input_kind == "pdf":
                ...
                if not text.strip() or is_garbled_text(text):
                    # Scanned or encoded-font PDF — render all pages and use vision model
                    with st.spinner("PDF text unreadable, converting to image for vision model..."):
                        try:
                            img_bytes, page_images = pdf_to_image_bytes(pdf_bytes, zoom=2.0)
                        except ImportError:
                            st.error("PyMuPDF is not installed. Run `pip install pymupdf`.")
                            st.stop()

                    # Display pages side by side in columns
                    ...
                    with st.spinner("Asking AI (vision) from PDF..."):
                        parsed_data = parse_receipt_from_image(image_bytes=img_bytes, mime="image/png")

                    # Override AI date: first try PyMuPDF text layer, then crop page for vision
                    reliable_date = extract_date_from_pdf_top(pdf_bytes)
                    if not reliable_date:
                        # Crop top AND bottom strips of page 1 and ask vision model for the date
                        try:
                            import fitz
                            from PIL import Image as PILImage
                            import io as _io
                            ...
                            pix = page.get_pixmap(matrix=mat)
                            full_img = PILImage.open(_io.BytesIO(pix.tobytes("png")))
                            doc.close()
                            h, w = full_img.height, full_img.width
                            # Stitch top 15% and bottom 30% into one image
                            top_strip = full_img.crop((0, 0, w, int(h * 0.15)))
                            bot_strip = full_img.crop((0, int(h * 0.70), w, h))
                            combined = PILImage.new("RGB", (w, top_strip.height + bot_strip.height), (255, 255, 255))
                            combined.paste(top_strip, (0, 0))
                            combined.paste(bot_strip, (0, top_strip.height))
                            buf = _io.BytesIO()
                            combined.save(buf, format="PNG")
                            crop_bytes = buf.getvalue()
                            crop_url = base64_data_url(crop_bytes, "image/png")
                            date_resp = client.responses.create(
                                model=MODEL_VISION,
                                input=[{
                                    "role": "user",
                                    "content": [
                                        {"type": "input_text", "text": "What is the purchase date shown in this receipt image? Return ONLY the date in YYYY-MM-DD format, nothing else. The date may appear as M/D/YY (e.g. '2/27/26' = 2026-02-27) or MM/DD/YYYY (e.g. '02/23/2026')."},
                                        {"type": "input_image", "image_url": crop_url},
                                    ],
                                }],
                                temperature=0.0,
                            )
                            date_text = date_resp.output_text.strip()
                            date_match = re.search(r"\d{4}-\d{2}-\d{2}", date_text)
                            if date_match:
                                reliable_date = date_match.group(0)
                        except Exception:
                            pass

                    if reliable_date:
                        parsed_data["receipt_date"] = reliable_date
                        st.info(f"Date extracted from PDF: **{reliable_date}**")
                else:
...
if (
    ...
    st.markdown("---")
    if st.button("SAVE TO DATABASE (Permanent)", type="primary", width="stretch"):
...
if os.path.exists(DB_PATH) and inspect(engine).has_table("receipts"):
    ...
    if not items.empty:
        ...
        col1.dataframe(cat.reset_index().style.format({"price": "${:,.2f}"}), width="stretch")
        col2.plotly_chart(px.pie(cat, names=cat.index, values=cat.values, title="Category Breakdown"), width="stretch")
