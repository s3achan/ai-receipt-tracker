...
def compute_tax(parsed_data: dict, items: list[dict]) -> tuple[dict, list[dict]]:
    ...
    taxable_items = [it for it in items if float(it.get("price") or 0) > 0]

    taxable_sum = sum(float(it.get("price") or 0) for it in taxable_items)
...
if uploaded_file is not None:
    ...
    if dup_id:
        st.warning(f"⚠️ This file looks like a duplicate of Receipt #{dup_id}. You can still save it if it's different.")
...
if input_kind and st.button("🔍 Parse & Categorize", type="primary"):
        ...
        if input_kind == "pdf":
            ...
            if not text.strip() or is_garbled_text(text):
                ...
                if reliable_date:
                    ...
                    st.info(f"📅 Date extracted: **{reliable_date}**")
            else:
...
if inspect(engine).has_table("receipt_items"):
    ...
    if not items.empty:
        ...
        col1.subheader("By Category")
        col1.dataframe(cat.reset_index().style.format({"price": "${:,.2f}"}), width="stretch")
        col2.plotly_chart(
            px.pie(cat, names=cat.index, values=cat.values, title="Category Breakdown"),
            width="stretch",
        )
        ...
        st.dataframe(
            monthly.rename(columns={"month_label":"Month","price":"Total"})
