...
def extract_date_from_pdf_bytes(pdf_bytes: bytes) -> str | None:
    """
    Try to extract a date from the PDF by scanning all pages text layers.
    Looks for patterns like 2/27/26 or 2/27/2026.
...
You must return:
- store (string)
- receipt_date (YYYY-MM-DD) — look carefully for ANY date on the receipt. For Costco web/PDF receipts the date is often printed at the very top of the page in M/D/YY format (e.g. "2/27/26" means 2026-02-27). Use that date. Do NOT invent or default a date.
- subtotal (number)
- tax (number)
- total (number)
        ...
        try:
            if input_kind == "pdf":
                ...
                if not text.strip() or is_garbled_text(text):
                    ...
                    # Override AI date with reliably extracted date from PDF text layer
                    reliable_date = extract_date_from_pdf_bytes(pdf_bytes)
                    if reliable_date:
